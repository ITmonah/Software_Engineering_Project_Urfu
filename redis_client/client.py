    ...
    try:
        ...
        for attempt in range(3):
            try:
                ...
                last_exc = e
                log(func_name, f"Попытка {attempt+1}: не удалось подключиться к Redis: {e}", "WARNING")
                await asyncio.sleep(1)
        
        log(func_name, f"Ошибка подключения к Redis после 3 попыток: {last_exc}", "ERROR")
        raise last_exc
...
def get_redis() -> redis.Redis:
    ...
    if redis_client is None:
        ...
        log("get_redis", "Ленивая инициализация Redis без проверки подключения", "WARNING")
